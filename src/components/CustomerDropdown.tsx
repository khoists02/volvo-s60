...
export interface ICustomer { icon?: React.ReactElement, ticker: string; name: string }
...
const CustomerDropdown: FC<ICustomerDropdown> = ({
    ...
    return (
        <div className="dropdown  w-sm-auto cursor-pointer">
            ...
            <div className={`dropdown-menu dropdown-menu-lg-end w-100 w-lg-auto wmin-300 wmin-sm-350 pt-0 ${show ? "show": ""}`}>
                {
                    options.map((op) => {
                        return (
                            <span key={op.ticker} onClick={() => {
                                onChange(op);
                                setShow(false);
                            }} className={`dropdown-item ${selected.ticker === op.ticker ? "active": ""}  py-2`}>
                                {op.icon}
                                <div>
